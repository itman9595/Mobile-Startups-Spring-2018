...
class Silver: UIViewController {
    ...
    @IBAction func buttonPressed(_ sender: UIButton) {
        if self.questions[index][sender.tag] == self.answers[index] {
            ...
            self.total += 1
            self.rightAnswers.text = "Правильные ответы: \(self.total)"
        } else {
            sender.layer.borderWidth = 3
            sender.layer.borderColor = UIColor.red.cgColor
            self.wrong += 1
            self.wrongAnswers.text = "Неправильные ответы: \(self.wrong)"
        }
        self.clicked()
    }
    ...
    @IBAction func submitButtonPressed(_ sender: UIButton) {
        ...
        if self.index < questions.count-1 {
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { (action) in
                ...
                self.total = 0
                self.wrong = 0
                self.rightAnswers.text = "Правильные ответы: 0"
                self.wrongAnswers.text = "Неправильные ответы: 0"
                self.getQuestions()
            }))
